...
def dm04_模型实现():
    ...
    data_l = load_iris()
    # 获取训练集,测试集
    x_train, x_test, y_train, y_test = train_test_split(data_l.data, data_l.target, test_size=0.2, random_state=18)
    # 实例话标准化对象
    ...
    # 通过叉验证和网格搜索api获得新模型
    model = GridSearchCV(model,parm_dict,cv = 5)
    # 训练模型
    model.fit(x_train,y_train)
    # 使用最优超参构建新模型
    model2 = KNeighborsClassifier(n_neighbors=model.best_params_['n_neighbors'])
    # 训练新模型
